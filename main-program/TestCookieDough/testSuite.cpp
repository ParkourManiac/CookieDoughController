...
void AddKeymapsFromPayloadIntoList_PayloadIsSplitBetweenStartAndEndOfStorage_SuccessfullyAddsKeymapsToList();
...
void RunTests() 
{
	...
	RUN_TEST(CalculateUnusedStorage_CalculatesTheAmountOfUnusedStorageAfterTheoreticallySavingTheGivenAmountOfKeymaps);
	RUN_TEST(CalculateUnusedStorage_NoArguments_CalculatesTheUnusedStorageAsIfWeWereToSaveDownTheCustomKeyMaps);
	RUN_TEST(CalculateUnusedStorage_NoArguments_ReturnsTheSameSizeAsIfWeWereToSaveKeymapsToMemory);
	RUN_TEST(CalculateUnusedStorage_TooManyKeymapsToFitOnTheEeprom_ReturnsTheOverloadAsANegativeNumber);
	...
	RUN_TEST(InsertKeymapsFromStorage_CorrectlyLoadsKeymapIntoList);
	RUN_TEST(InsertKeymapsFromStorage_CorrectlyLoadsKeymapIntoList_ReturnsTrue);
	RUN_TEST(InsertKeymapsFromStorage_LoadsKeymap_SetsCurrentPacketAdressToTheLoadedPacketsAdress);
	RUN_TEST(InsertKeymapsFromStorage_LoadsKeymap_SetsNextPacketAdressToAFreeAdressAfterTheEndOfTheLoadedPacket);
	RUN_TEST(InsertKeymapsFromStorage_LoadsKeymap_SetsTheAmountOfFreeStorageToBeEqualToTheStorageSizeMinusTheSizeOfTheLoadedPacket);
	RUN_TEST(InsertKeymapsFromStorage_GoesThroughWholeStorageWithoutFindingValidKeymaps_DoesNotRepeatInInfiniteLoop_AndReturnsFalse);
	RUN_TEST(InsertKeymapsFromStorage_OnlyOneInvalidKeymapInMemory_DoesNotAttemptToLoadTheSamePacketTwice_AndReturnsFalse);
	RUN_TEST(InsertKeymapsFromStorage_FailsToLoadKeymap_DoesNotChangeAmountOfFreeStorage);
	RUN_TEST(InsertKeymapsFromStorage_NoPacketInStorage_ReturnsFalse);
	RUN_TEST(InsertKeymapsFromStorage_LoadsKeymapThatExceedsTheEepromsLastAdress_DoesNotUpdateNextPacketAdressWithAnAdressOutsideTheEeprom);
	RUN_TEST(InsertKeymapsFromStorage_EepromHasDefectKeymaps_DoesNotLoadKeymaps);
	RUN_TEST(InsertKeymapsFromStorage_EepromHasDefectKeymapsFollowedByValidKeymaps_LoadsTheValidKeymaps);
	RUN_TEST(AddKeymapsFromPayloadIntoList_SuccessfullyAddsKeymapToList);
	RUN_TEST(AddKeymapsFromPayloadIntoList_AddsKeymapToList_ReturnsTrue);
	RUN_TEST(AddKeymapsFromPayloadIntoList_SuccessfullyAddsMultipleKeymapsToList);
	RUN_TEST(AddKeymapsFromPayloadIntoList_PayloadIsSplitBetweenStartAndEndOfStorage_SuccessfullyAddsKeymapsToList);
	RUN_TEST(AddKeymapsFromPayloadIntoList_OneKeymapFails_RemovesKeymapsThatWereAddedFromFaultyPayload);
	RUN_TEST(AddKeymapsFromPayloadIntoList_FailsToReadPayload_ReturnsFalse);
	RUN_TEST(AddKeymapsFromPayloadIntoList_KeymapInPayloadIsInvalid_ReturnsFalse);
	...
	RUN_TEST(CreateNewKeymap_SramDoesNotFitAnotherKeymap_DoesNotCreateNorChangeKeymapAndReturnsFalse);
	...
	RUN_TEST(Helper_IsPacketValidOnEEPROM_PrepareToReadPacket_ReadsValidPacketFromEepromSuccessfullyAndReturnsTrueAndOutputsCorrectValues);
	RUN_TEST(Helper_IsPacketValidOnEEPROM_PrepareToReadPacket_SetEepromSizeByHandToFitThePacket_SuccessfullyValidatesThePacket);
	RUN_TEST(Helper_IsPacketValidOnEEPROM_PrepareToReadPacket_UsingHighAdress_EepromLengthIsSetToFitPacket);
	RUN_TEST(Helper_IsPacketValidOnEEPROM_PrepareToReadPacket_SetEepromSizeByHandToNotFitThePacket_ValidationOfPacketFails);
	RUN_TEST(Helper_IsPacketValidOnEEPROM_PrepareToReadPacket_RecievesInactiveFlag_ReturnsFalse);
	RUN_TEST(Helper_ReadBytesFromEEPROM_PreparesToReadPayload_RecievesValidPacket_OutputsThePayloadAndReturnsTrue);
	RUN_TEST(Helper_ReadBytesFromEEPROM_PreparesToReadPayload_SetEepromToBeLargerThanPacket_SuccessfullyReturnsPayload);
	RUN_TEST(Helper_ReadBytesFromEEPROM_PreparesToReadPayload_UsingHighAdress_EepromLengthIsAutomaticallySetToFitPacket);
	RUN_TEST(Helper_ReadBytesFromEEPROM_PreparesToReadPayload_SetEepromToBeSmallerThanPacket_ReturnsFalse);
	RUN_TEST(Helper_ReadBytesFromEEPROM_PreparesToReadPayload_EepromSizeIsSmallerThanRequestedBytes_ReturnsFalse);
	...
	RUN_TEST(CalculateCRC_DoesNotAlterInput);
	...
	RUN_TEST(SizeOfEmptySerializedDataPacket_ReturnsTheAmountOfBytesAnEmptyDataPacketWouldOccupyOnMemory);
	RUN_TEST(SizeOfSerializedDataPacket_ReturnsTheAmountOfBytesTheDataPacketWillOccupyOnMemory);
	RUN_TEST(SizeOfSerializedDataPacket_RecievesPacketWithoutAPayload_StillCalculatesSizeOfRemainingPacket);
	...
	RUN_TEST(ReadBytesFromEEPROM_BeginsReadingAtTheAdress);
	RUN_TEST(ReadBytesFromEEPROM_ReadsTheRequestedAmountOfBytes);
	RUN_TEST(ReadBytesFromEEPROM_ReadsBytesFromTheCorrectAdressUsingACyclicPattern);
	RUN_TEST(ReadBytesFromEEPROM_FillsTheOutputArrayWithTheCorrectBytesFromEEPROM);
	RUN_TEST(ReadBytesFromEEPROM_FillsTheOutputArrayWithTheRequestedAmountOfBytes);
	RUN_TEST(ReadBytesFromEEPROM_SuccessfullyReadsASeriesOfBytes_ReturnsTrue);
	...
	RUN_TEST(IsPacketOnEEPROMValid_BeginsReadingAtTheAdress);
	RUN_TEST(IsPacketOnEEPROMValid_PacketIsValid_ReturnsTrue);
	RUN_TEST(IsPacketOnEEPROMValid_ReadsThePacketInACylicFormat);
	RUN_TEST(IsPacketOnEEPROMValid_WithOutputArguments_ReturnsPayloadsAdress);
	RUN_TEST(IsPacketOnEEPROMValid_WithOutputArguments_ReturnsThePayloadsLength);
	RUN_TEST(IsPacketOnEEPROMValid_WithOutputArguments_ReturnsTheCorrectPacketSize);
	RUN_TEST(IsPacketOnEEPROMValid_AdressIsOutsideOfTheEEPROMsRange_ReturnsFalse);
	RUN_TEST(IsPacketOnEEPROMValid_CanNotFindStx_ReturnsFalse);
	RUN_TEST(IsPacketOnEEPROMValid_PacketIsDeactivated_ReturnsFalse);
	RUN_TEST(IsPacketOnEEPROMValid_PayloadLengthIsZero_ReturnsFalse);
	RUN_TEST(IsPacketOnEEPROMValid_PacketFitsOnEEPROM_ReturnsTrue);
	RUN_TEST(IsPacketOnEEPROMValid_PayloadLengthIsLargerThanWhatFitsOnTheEEPROM_ReturnsFalse);
	RUN_TEST(IsPacketOnEEPROMValid_CanNotFindEtx_ReturnsFalse);
	RUN_TEST(IsPacketOnEEPROMValid_CrcDoesNotMatchPayload_ReturnsFalse);
	...
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_ReadsThePacketUsingACircularPatternAndReadsFromTheCorrectAdress);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_SuccessfullyAddsTheDataToTheEndOfThePayload);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_SuccessfullyAddsAnEtxToTheEndOfThePayload);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_SuccessfullyIncreasesThePayloadLength);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_SuccessfullyIncludesTheDataIntoTheCalculationOfTheCrc);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_WritesThePayloadInACircularPatternWhenExceedingTheEepromsLastAdress);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_StxIsAtTheLastPostitionOfTheEeprom_WritesEverythingInACircularPatternAtTheCorrectPlace);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_JoinsDataWithPacket_ReturnsTrue);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_TheJoinedPacketWillBeBiggerThanTheEeprom_DoesNotWriteToEEPROMAndReturnsFalse);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_AdressExceedsTheLastAdressOfTheEEPROM_ReturnsFalse);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_AdressDoesNotPointToAnStx_ReturnsFalse);
	RUN_TEST(JoinDataWithDataPacketOnEEPROM_CanNotFindEtx_ReturnsFalse);
	...
	RUN_TEST(FindFirstDataPacketOnEEPROM_FindsPacket_ReturnsCorrectPayloadAdress);
	RUN_TEST(FindFirstDataPacketOnEEPROM_FindsPacket_ReturnsCorrectPayloadLength);
	...
	RUN_TEST(DataPacketWriter_Constructor_IsCompletedVariableIsSetToFalse);
	RUN_TEST(DataPacketWriter_Constructor_AddressVariableIsSetToTheGivenPacketAdress);
	RUN_TEST(DataPacketWriter_Constructor_PayloadLengthVariableIsSetToZero);
	RUN_TEST(DataPacketWriter_Constructor_SizeOfEepromVariableIsSetToTheSizeOfTheEEPROM);
	RUN_TEST(DataPacketWriter_Constructor_WritesStxToGivenAdress);
	RUN_TEST(DataPacketWriter_Constructor_WritesStxToGivenAdress_SuccessVariableIsSetToTrue);
	RUN_TEST(DataPacketWriter_Constructor_WroteDownStx_AddsSizeOfStxToPacketSizeVariable);
	...
	RUN_TEST(AddDataToPayload_PacketBecomesTooBigForEEPROM_DoesNotWritePayloadToMemoryAndReturnsFalse);
	...
	RUN_TEST(AddDataToPayload_PayloadAndStxIsCorrectlyPutDown);
	RUN_TEST(AddDataToPayload_AddsMultipleParts_PayloadAndStxIsCorrectlyPutDown);
	...
	RUN_TEST(AddDataToPayload_AddsMultipleParts_AddsSizeOfEachDataPartToPayloadLengthVariable);
	RUN_TEST(AddDataToPayload_WroteDownData_AddsSizeOfWrittenDataToPacketSizeVariable);
	RUN_TEST(AddDataToPayload_AddsMultipleParts_AddsSizeOfEachDataPartToPacketSizeVariable);
	RUN_TEST(AddDataToPayload_WroteDownData_CalculatesCrcOfPayload);
	RUN_TEST(AddDataToPayload_WroteDownMultipleDataParts_AddsEachPartToCrc);
	RUN_TEST(AddDataToPayload_DataPacketWriterIsUnsuccessful_DoesNotWriteToStorageAndReturnsFalse);
	...
	RUN_TEST(FinishWritingPacket_WritesPayloadLengthToCorrectAddress);
	RUN_TEST(FinishWritingPacket_PayloadLengthsAddressExceedsStorage_WritesPayloadLengthAtTheStartOfTheStorage);
	RUN_TEST(FinishWritingPacket_Succeeds_PayloadLengthVariableHasTheCorrectValue);
	...
	RUN_TEST(FinishWritingPacket_Succeeds_CrcVariableHasTheCorrectValue);
	...
	RUN_TEST(FinishWritingPacket_EtxIsPutDownAtTheEndOfThePacket);
	...
	RUN_TEST(FinishWritingPacket_PacketWillExceedEndOfEEPROM_ReturnsCorrectPacketSize);
	RUN_TEST(FinishWritingPacket_PacketIsCorrectlyPutDownAndReturnsTrue);
	RUN_TEST(FinishWritingPacket_WritesMultipleDataPartsToPayload_PacketIsCorrectlyPutDown);
	RUN_TEST(FinishWritingPacket_PacketWillExceedEndOfEEPROM_SplitsPacketBetweenEndAndStartOfEEPROM);
	RUN_TEST(FinishWritingPacket_Succeeds_IsCompletedIsSetToTrue);
	RUN_TEST(FinishWritingPacket_Succeeds_SuccessVariableIsTrue);
	RUN_TEST(FinishWritingPacket_SuccessIsFalse_ReturnsFalseAndDoesNotWriteToStorage);
	RUN_TEST(FinishWritingPacket_PacketIsAlreadyCompleted_ReturnsFalseAndDoesNotWriteToStorage);
	RUN_TEST(FinishWritingPacket_PacketCannotBeReadFromStorage_SuccessIsFalseAndReturnsFalse);
	RUN_TEST(FinishWritingPacket_PacketIsSavedButEepromFailsToReadTheData_SuccessIsFalseAndReturnsFalse);
